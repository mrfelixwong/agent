...
class MockAudioRecorder(AudioRecorder):
    """
    Mock audio recorder for testing without actual audio hardware
    """
    
    def __init__(self, **kwargs):
        """Initialize mock recorder"""
        # Don't call parent __init__ to avoid PyAudio initialization
        ...
        self.audio_format = kwargs.get('audio_format', "16bit")
    ...
    def start_recording(self, output_file_path: str, device_index: Optional[int] = None) -> bool:
        ...
        if self._simulate_error:
            self._handle_error(Exception("Simulated recording error"))
            return False
    ...
    def stop_recording(self) -> Dict[str, Any]:
        ...
        end_time = time.time()
        duration = self._mock_duration if self._mock_duration > 0 else (end_time - self.start_time)
